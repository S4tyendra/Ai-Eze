...
enum class PromptTemplate(val displayName: String, val template: String) {
    ...
    companion object {
        fun getCommonActions() = listOf(
            REPHRASE, FIX_GRAMMAR, MODIFY_TONE_FORMAL, SUGGEST_REPLY
        )

        fun getToneActions() = listOf(
            MODIFY_TONE_FORMAL, MODIFY_TONE_CASUAL, MODIFY_TONE_POLITE
        )

        fun getTranslationActions() = listOf(
            TRANSLATE_TO_HINDI, TRANSLATE_TO_ENGLISH
        )

        fun getFormattingActions() = listOf(
            SUMMARIZE, EXPAND, MAKE_BULLET_POINTS, EXPLAIN_LIKE_IM_5
        )
    }
}
